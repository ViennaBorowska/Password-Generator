...
var generatePassword = function () {
  ...
  if (parseInt(lengthPmt) >= 8 || parseInt(lengthPmt) <= 128) {
    ...
    var syms = confirm(
      "Should special characters e.g. !@#$%^&*(){}[]=<>/,. be included?"
    );
    if (syms == true) {
      window.alert(
        "Special characters will be included. Your password will now be generated..."
      );
      allowed = allowed.concat(useSyms);
      console.log(useSyms);
    } else {
      window.alert(
        "Special characters will not be used. Your password will now be generated..."
      );
    }
  }
  ...
  if (!upperC && !lowerC && !nums && !syms) {
    alert(
      "You must select at least one character option to generate your password!"
    );
    return null;
  }
